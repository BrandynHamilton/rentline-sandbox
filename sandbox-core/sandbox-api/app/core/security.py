...
class APIKeyMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        try:
            ...
            # Admin key passes everything; set a synthetic clerk_user_id so
            # game routes that call _clerk_id() work without Clerk JWT.
            if ADMIN_API_KEY and api_key == ADMIN_API_KEY:
                if not getattr(request.state, "clerk_user_id", None):
                    request.state.clerk_user_id = "admin_api_key"
                return await call_next(request)
