...
class SandboxGame(Base):
    ...
    # Maximum number of turns. Each turn = one turn_duration period.
    # Monthly: 12 = 1 year, 120 = 10 years, 1200 = 100 years.
    # Yearly:  10 = 10 years, 100 = 100 years.

    turn_duration: Mapped[str] = mapped_column(String, default="month", nullable=False)
    # "month" — each turn represents one calendar month (default, most granular)
    # "year"  — each turn represents one year (faster games, fewer debt service cycles)
    # Affects: Fed meeting interval interpretation, lease renewal rates, and feed descriptions.
    # Does NOT change the engine mechanics — it's a label/context field for players/UI.

    turn_duration_seconds: Mapped[int] = mapped_column(Integer, default=1800, nullable=False)
    # Real-world wall-clock seconds each player has to act before the turn auto-advances.
    # Default: 1800 = 30 minutes. Set to 0 to disable the deadline (manual advance only).
    # The runner checks: if all players are ready → advance immediately.
    # If deadline elapsed and not all ready → advance anyway (idlers skipped).
    ...
    judgment_on_shortfall: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    # When True, any deficiency remaining after selling a mortgaged property (sale proceeds < loan
    # balance) is recorded as a judgment lien on the player (reduces NAV for the rest of the game).
    # When False (default), shortfalls are absorbed — the player takes a cash loss but carries no
    # further liability beyond what their balance allows.

    # Property improvement / PACE config
    upgrade_cost_pct: Mapped[float] = mapped_column(Float, default=0.08, nullable=False)
    # Cost per grade step as % of current price (default 8%). D→C = 1 step × 8% × price.
    improvement_value_add_pct: Mapped[float] = mapped_column(Float, default=0.05, nullable=False)
    # One-time price bump per grade step when improvement completes (default 5%).
    pace_spread: Mapped[float] = mapped_column(Float, default=0.015, nullable=False)
    # PACE lien rate = base_mortgage_rate + pace_spread (default +1.5%).
    ...
    # Seconds between turns for all-bot autonomous games (fast mode). Ignored when
    # turn_duration_seconds > 0 and there are human players.

    turn_started_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    # Timestamp when the current trading window opened (set at end of advance_turn).
    # Used by the runner to determine when the turn_duration_seconds deadline has elapsed.
...
class SandboxPlayer(Base):
    ...
    # Agent delegation — human players can opt-in to have an LLM act on their behalf
    # if they haven't taken any action before the turn advances.
    agent_delegate: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    # True = run a bot turn for this player if they haven't acted this trade window
    delegate_strategy: Mapped[str | None] = mapped_column(String, nullable=True)
    # Which bot strategy to use when acting as delegate (default: "balanced")
    last_action_turn: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    # The last turn on which this player took an action (trade, mortgage, heloc, or mark_ready).
    # Used to detect idleness: idle if last_action_turn < game.current_turn.

    # Judgment debt — only non-zero when game.judgment_on_shortfall=True and player sold a
    # mortgaged property underwater. Persists as a NAV liability for the rest of the game.
    judgment_balance: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)
...
class SandboxProperty(Base):
    ...
    initial_grade: Mapped[str] = mapped_column(String, default="C", nullable=False)
    # Explicit property condition grade set at pool sync time.
    # A=excellent, B=good, C=average, D=below average, F=distressed.
    # Used to seed SandboxGameProperty.grade at game creation.
    # When sourced from rwa-issuer-sim, pass "initial_grade" field in the API response.
    # When seeded from built-in data, set explicitly per property.
...
class SandboxGameProperty(Base):
    ...
    # Property grade — A (premium) through F (distressed). Seeded from cap_rate at game
    # creation. Improves when a player funds an improvement or PACE lien. Affects
    # appreciation probability, rent multiplier, vacancy risk, and capex probability.
    grade: Mapped[str] = mapped_column(String, default="C", nullable=False)
    # "A" | "B" | "C" | "D" | "F"

    # Mechanics lien — non-zero when a contractor/price dispute is active on this property.
    # Blocks refi and PACE origination until paid off. Cleared via prepay_principal
    # targeting mortgage_type="mechanics_lien", or expires after N turns.
    mechanics_lien_amount: Mapped[float] = mapped_column(Float, default=0.0, nullable=False)

    # EVM address of the PropertyToken contract for this property — carried through from
    # SandboxProperty.token_address at game creation. Surfaced in game API responses so
    # clients can link holdings to the on-chain token and Fuji explorer.
...
class SandboxHolding(Base):
    ...
    acquired_turn: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    # Game turn on which the player first acquired tokens for this holding.
    # Used to compute turns_held and annualised yield in portfolio response.
...
class SandboxMacroEvent(Base):
    ...
    # pending  — announced but not yet active (rate macros get 1 warning turn before taking effect)
    # expired  — duration exhausted
