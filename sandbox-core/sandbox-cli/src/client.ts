...
export function createClient(opts: RequestOptions) {
  ...
  return {
    ...
    prepayPrincipal: (gameId: string, body: PrepayPrincipalBody) =>
      r<Mortgage>("POST", `/api/sandbox/games/${gameId}/prepay-principal`, body),
    improveProperty: (gameId: string, body: ImprovePropertyBody) =>
      r<Record<string, unknown>>("POST", `/api/sandbox/games/${gameId}/improve-property`, body),
    originatePaceLien: (gameId: string, body: PaceLienBody) =>
      r<Mortgage>("POST", `/api/sandbox/games/${gameId}/pace-lien`, body),
    ...
    // ── Agent delegation ──────────────────────────────────────────────────────
    setDelegate: (gameId: string, body: DelegateBody) =>
      r<DelegateResult>("POST", `/api/sandbox/games/${gameId}/delegate`, body),

    // ── API keys ──────────────────────────────────────────────────────────────
    createApiKey: (name = "CLI key") =>
      r<ApiKeyResult>("POST", "/api/sandbox/api-keys", { name }),

    // ── Market & Intel ────────────────────────────────────────────────────────
    getMarketSummary: (gameId: string) =>
      r<Record<string, unknown>>("GET", `/api/sandbox/games/${gameId}/market-summary`),
    getPlayerActions: (gameId: string, playerId: string, limit?: number, turn?: number) =>
      r<Record<string, unknown>[]>("GET", `/api/sandbox/games/${gameId}/players/${playerId}/actions${limit || turn ? `?${new URLSearchParams({...(limit ? {limit: String(limit)} : {}), ...(turn !== undefined ? {turn: String(turn)} : {})})}` : ""}`),
    spectate: (gameId: string) =>
      r<Record<string, unknown>>("GET", `/api/sandbox/games/${gameId}/spectate`),
    createGameFromPreset: (body: PresetGameBody) =>
      r<Record<string, unknown>>("POST", "/api/sandbox/games/from-preset", body),
  };
}
...
export interface PrepayPrincipalBody { property_id: string; amount: number; mortgage_type?: "first_lien" | "acquisition" | "refi" | "heloc" | "pace" | "mechanics_lien"; }
export interface ImprovePropertyBody { property_id: string; target_grade: "A" | "B" | "C" | "D"; }
export interface PaceLienBody { property_id: string; target_grade: "A" | "B" | "C" | "D"; }
export interface PresetGameBody { preset: string; name: string; display_name: string; starting_balance_usdc?: number; }
...
export interface DelegateBody { agent_delegate: boolean; delegate_strategy?: string; }
export interface DelegateResult { player_id: string; agent_delegate: boolean; delegate_strategy: string; message: string; }
export interface ApiKeyResult { id: string; name: string; key_prefix: string; created_at: string; expires_at: string | null; raw_key?: string; }
