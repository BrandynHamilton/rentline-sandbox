...
_SCRIPT_DIR    = Path(__file__).parent          # backend/scripts/
_ROOT_DIR      = _SCRIPT_DIR.parent.parent      # project root
...
def verify_contract(name: str, addr: str, ctor_args_hex: str = ""):
    """
    Submit a contract for verification on Snowtrace (Fuji).

    name          — contract name matching the Foundry artifact (e.g. "PropertyToken")
    ...
    # Give Snowscan time to index the newly deployed contract
    time.sleep(10)
